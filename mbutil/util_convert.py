...
def convert_tile_to_bbox(z, x, y, **kwargs):
    if kwargs.get('flip_y', False) == True:
        y = flip_y(z, y)

    left, bottom = tile_to_coordinate(x - 0.5, y + 0.5, z)
    right, top   = tile_to_coordinate(x + 0.5, y - 0.5, z)

    sys.stdout.write("%f,%f,%f,%f\n" % (left, bottom, right, top))


def tiles_for_bbox(left, bottom, right, top, zoom, flip_y):
...
def convert_string(conversion_string, **kwargs):
	...
	if match:
		convert_tile_to_bbox(int(match.group(1)), int(match.group(2)), int(match.group(3)), **kwargs)
		return
